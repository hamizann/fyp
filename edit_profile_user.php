...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = $con->prepare("UPDATE user_account SET user_username = ?, user_fullname = ?, user_email = ?, user_phoneNo = ?, user_dateOfBirth = ? WHERE user_id = ?");

    if ($query) {
        $query->bind_param('sssssi', $username, $fullname, $email, $phone, $dob, $userId);
        $query->execute();

        // Redirect back to the profile page after updating
        header('Location: profile_page_user.php');
        exit();
    } else {
    ...
    if ($query) {
        ...
        if ($result->num_rows > 0) {
            ...
            echo "       <input type='text' class='form-control' id='username' name='username' value='" . $user['user_username'] . "'>";
            echo "   </div><br>";
            echo "   <div class='form-group'>";
            echo "       <strong><label for='fullname'>Name:</label></strong>";
            echo "       <input type='text' class='form-control' id='fullname' name='fullname' value='" . $user['user_fullname'] . "'>";
            echo "   </div><br>";
            echo "   <div class='form-group'>";
            echo "       <strong><label for='email'>Email:</label></strong>";
            echo "       <input type='email' class='form-control' id='email' name='email' value='" . $user['user_email'] . "'>";
            echo "   </div><br>";
            echo "   <div class='form-group'>";
            echo "       <strong><label for='phone'>Phone:</label></strong>";
            echo "       <input type='tel' class='form-control' id='phone' name='phone' value='" . $user['user_phoneNo'] . "'>";
            echo "   </div><br>";
            echo "   <div class='form-group'>";
            echo "       <strong><label for='dob'>DOB:</label></strong>";
            echo "       <input type='date' class='form-control' id='dob' name='dob' value='" . $user['user_dateOfBirth'] . "'>";
            echo "   </div><br>";
